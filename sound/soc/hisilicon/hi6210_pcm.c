...
static irq_rt_t hi6210_notify_recv_isr(void *usr_para, void *mail_handle, unsigned int mail_len)
{
	...
	switch(mail_buf.msg_type) {
		...
		case HI_CHN_MSG_PCM_PERIOD_STOP:
			...
			break;
		case HI_CHN_MSG_PCM_XRUN:
			logi("pcm xrun \n");
			prtd->hw_ptr_interrupt_last = substream->runtime->hw_ptr_interrupt;
			mod_ret = mod_delayed_work(prtd->xrun_check_delay_wq,
				&prtd->xrun_check_delay_work,
				msecs_to_jiffies(200));
			if (mod_ret != 0) {
				loge("delaywork start err %d \n", mod_ret);
			}
			break;
		default:
			loge("msg_type 0x%x\n", mail_buf.msg_type);
...
static int hi6210_pcm_close(struct snd_pcm_substream *substream)
{
	...
	case PCM_DEVICE_NORMAL:
		if(SNDRV_PCM_STREAM_PLAYBACK == substream->stream) {
			...
			spin_unlock_bh(&g_pcm_pb_open_lock);
			cancel_delayed_work(&pdata.pcm_rtd_playback.xrun_check_delay_work);
			flush_workqueue(pdata.pcm_rtd_playback.xrun_check_delay_wq);
		} else {
			spin_lock_bh(&g_pcm_cp_open_lock);
			pdata.pcm_cp_status_open = (u32)0;
			ret = hi6210_pcm_notify_close(substream);
			spin_unlock_bh(&g_pcm_cp_open_lock);
			cancel_delayed_work(&pdata.pcm_rtd_capture.xrun_check_delay_work);
			flush_workqueue(pdata.pcm_rtd_capture.xrun_check_delay_wq);
		}
		break;
	case PCM_DEVICE_DIRECT:
		if(SNDRV_PCM_STREAM_PLAYBACK == substream->stream) {
			...
			ret = hi6210_pcm_notify_close(substream);
			spin_unlock_bh(&g_pcm_pb_open_lock);
			cancel_delayed_work(&pdata.pcm_rtd_direct_playback.xrun_check_delay_work);
			flush_workqueue(pdata.pcm_rtd_direct_playback.xrun_check_delay_wq);
		}
		break;
	case PCM_DEVICE_FAST:
		if(SNDRV_PCM_STREAM_PLAYBACK == substream->stream) {
...
void xrun_check_delay_wq(struct work_struct *work)
{
	int ret = 0;
	struct snd_pcm_runtime *runtime = NULL;
	snd_pcm_sframes_t avail = 0;
	struct hi6210_runtime_data *priv = container_of(work, struct hi6210_runtime_data, xrun_check_delay_work.work);

	if ((priv->status == 1) && priv->substream) {
		runtime = priv->substream->runtime;
		if (runtime->hw_ptr_interrupt == priv->hw_ptr_interrupt_last) {
			++priv->period_cur;
			priv->period_cur = (priv->period_cur) % runtime->periods;
			priv->period_next = (priv->period_next + 1) % runtime->periods;
			snd_pcm_period_elapsed(priv->substream);
			avail = snd_pcm_playback_avail(runtime);
			priv->hw_ptr_interrupt_last = runtime->hw_ptr_interrupt;
			logi("state %x avail %lu  avail_min %lu hw_ptr_interrupt %lu \n", runtime->status->state, avail, runtime->control->avail_min, runtime->hw_ptr_interrupt);
		}

		ret = mod_delayed_work(priv->xrun_check_delay_wq,
				&priv->xrun_check_delay_work,
				msecs_to_jiffies(200));
		if (ret != 0)
			loge("delaywork start err %d \n", ret);
	}
...
static int  hi6210_platform_probe(struct platform_device *pdev)
{
	...
	pdata.pcm_rtd_playback.xrun_check_delay_wq = create_singlethread_workqueue("playback_xrun_check_delay_wq");
	if (!(pdata.pcm_rtd_playback.xrun_check_delay_wq)) {
		loge("playback workqueue create failed");
		goto playback_wq_failed;
	}
	INIT_DELAYED_WORK(&pdata.pcm_rtd_playback.xrun_check_delay_work, xrun_check_delay_wq);

	pdata.pcm_rtd_capture.xrun_check_delay_wq = create_singlethread_workqueue("capture_xrun_check_delay_wq");
	if (!(pdata.pcm_rtd_capture.xrun_check_delay_wq)) {
		loge("capture workqueue create failed");
		goto capture_wq_failed;
	}
	INIT_DELAYED_WORK(&pdata.pcm_rtd_capture.xrun_check_delay_work, xrun_check_delay_wq);

	pdata.pcm_rtd_direct_playback.xrun_check_delay_wq = create_singlethread_workqueue("direct_xrun_check_delay_wq");
	if (!(pdata.pcm_rtd_direct_playback.xrun_check_delay_wq)) {
		loge(" direct workqueue create failed");
		goto direct_wq_failed;
	}
	INIT_DELAYED_WORK(&pdata.pcm_rtd_direct_playback.xrun_check_delay_work, xrun_check_delay_wq);
...
direct_wq_failed:
	cancel_delayed_work(&pdata.pcm_rtd_capture.xrun_check_delay_work);
	flush_workqueue(pdata.pcm_rtd_capture.xrun_check_delay_wq);
	destroy_workqueue(pdata.pcm_rtd_capture.xrun_check_delay_wq);
	pdata.pcm_rtd_capture.xrun_check_delay_wq = NULL;
capture_wq_failed:
	cancel_delayed_work(&pdata.pcm_rtd_playback.xrun_check_delay_work);
	flush_workqueue(pdata.pcm_rtd_playback.xrun_check_delay_wq);
	destroy_workqueue(pdata.pcm_rtd_playback.xrun_check_delay_wq);
	pdata.pcm_rtd_playback.xrun_check_delay_wq = NULL;
playback_wq_failed:
