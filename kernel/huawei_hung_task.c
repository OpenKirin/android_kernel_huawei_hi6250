...
static void watchdog_nosched_check(void)
{
	...
	if (hungtask_watchdog_nosched_status) {
		pr_err("hungtask: kick vmreboot watchdog");
					...
					const char *buf, size_t count)
{
	...
	if (strncmp(tmp, "on", strlen(tmp)) == 0) {
		...
		pr_info("hungtask: watchdog_nosched_enable is set to enable.\n");
	} else if (unlikely(strncmp(tmp, "off", strlen(tmp)) == 0)) {
		hungtask_watchdog_nosched_enable = 0;
		pr_info("hungtask: watchdog_nosched_enable is set to disable.\n");
