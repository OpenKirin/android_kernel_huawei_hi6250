...
int check_batt_terminate_too_early(char *buf)
{
	...
	if (di->chargedone_stat && capacity <= CHG_CANT_FULL_THRESHOLD) {
		...
		snprintf(buf, DSM_BATTERY_MAX_SIZE, "battery: %s, charger_type = %d, batt_volt = %dmV, batt_curr = %dmA, "
			...
			batt_brand, charger_type, volt, current_now, avg_curr, capacity, rm);
		return 1;
	}
	return 0;
}
...
int check_batt_bad_curr_sensor(char *buf)
{
	...
	if (times == DSM_CHECK_TIMES_LV2) {
		times = 0;
		avg_curr = hisi_battery_current_avg();
		snprintf(buf, DSM_BATTERY_MAX_SIZE, "current = %dmA, avg_curr = %dmA\n", current_now, avg_curr);
		return 1;
	}
	return 0;
}
...
int check_charge_temp_fault(char *buf)
{
	...
	if ((HIGH_TEMP <= batt_temp || LOW_TEMP > batt_temp) && (di->usb_online || di->ac_online)) {
		...
		usb_temp = get_usb_ntc_temp();
#endif
		snprintf(buf, DSM_BATTERY_MAX_SIZE, "temp fault cause not charging, batt_temp = %d, usb_temp = %d\n", batt_temp,usb_temp);
		return 1;
	}
#endif
	return 0;
...
int check_charge_batt_shutdown_temp(char *buf)
{
	...
	if (SHUTDOWN_TEMP <= temp) {
		...
		usb_temp = get_usb_ntc_temp();
#endif
		snprintf(buf, DSM_BATTERY_MAX_SIZE, "shutdown because batt_temp = %d, usb_temp = %d\n", temp,usb_temp);
		return 1;
	}
#endif
	return 0;
...
int check_charge_batt_capacity(char *buf)
{
	...
	if ((VCHRG_CHARGE_DONE_EVENT == di->event)
		...
		&& (batt_temp > LOW_TEMP)) {
		charger_type = charge_get_charger_type();
		snprintf(buf, DSM_BATTERY_MAX_SIZE, "battery: %s, batt_vol = %dmV, capacity = %dmAh, full capacity = %dmAh, charger_type = %d \n",
			hisi_battery_brand(), di->bat_voltage, di->bat_rm, fcc_design, charger_type);
		return 1;
	}
	return 0;
}
