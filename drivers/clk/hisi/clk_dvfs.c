...
static void updata_freq_volt_up_fn(struct work_struct *work)
{
	...
	if(target_freq > freq_old){
		...
		if(0 != ret)
			...
			ret = clk_set_rate_nolock(friend_clk, target_freq);
			if (ret < 0)
					       ...
					       unsigned long parent_rate)
{
	...
	if(strncmp(clk_name, "clk_vdec", sizeof("clk_vdec"))){
...
static int peri_dvfs_set(struct peri_dvfs_clk *dfclk, unsigned long rate, unsigned int volt)
{
	...
	/*ÅÐ¶ÏÆµÂÊ£¬ÅÐ¶ÏÊÇÉýÆµ»¹ÊÇ½µÆµ
        ÉýÆµ£ºÏÈÉýÑ¹£¬ºóÉèÆµ£» ½µÆµ£ºÏÈ½µÆµ£¬ÔÙÉèÑ¹*/
	if(rate > freq_old){
		/*unblock mode*/
...
{
    peri_volt_change(pclk->id, PERI_VOLT_0);
}
#endif
