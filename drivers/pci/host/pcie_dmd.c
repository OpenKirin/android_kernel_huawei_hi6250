...
void dsm_pcie_dump_info(struct kirin_pcie *pcie, enum dsm_err_id id)
{
	...
	if (!pcie || !pcie->is_power_on)
		return;

	dsm_record_info[i++] = id;
	dsm_record_info[i++] = gpio_get_value(pcie->gpio_id_reset);
	dsm_record_info[i++] = kirin_phy_readl(pcie, SOC_PCIEPHY_STATE0_ADDR);
...
void dsm_pcie_dump_info(struct kirin_pcie *pcie, const char *fmt, ...)
{
	return;
}
...
EXPORT_SYMBOL_GPL(dsm_pcie_log_addr);
#endif
