...
void hisp_rproc_resource_cleanup(struct rproc *rproc)
{
	...
	list_for_each_entry_safe(entry, tmp, &rproc->reserved_mems, node) {
            ...
            pr_info("use_nonsec_isp: %d, use_sec_isp: %d, reserved_mems : %s, isp_share_para: %pK\n",
                use_nonsec_isp(), use_sec_isp(), (char *)entry->priv, isp_share_para);/*lint !e559 */
	...
	list_for_each_entry_safe(page_entry, page_tmp, &rproc->pages, node) {
		...
		while (NULL != tmp_page && i < page_entry->num) {
			if (NULL != *tmp_page)
				...
				pr_info("tmp_page: %p, i: %d, page_num: %u", tmp_page, i, page_entry->num);
			tmp_page++;
			i++;
		}
		vfree(page_entry->va);
...
static void free_mdc_ion(void)
{
    struct rproc_boot_device *dev = &rproc_boot_dev;
    struct hisi_isp_ion_s *hisi_mdc_ion = dev->mdc_mem_info;

    if(hisi_mdc_ion == NULL){
        return;
    }
    ion_unmap_kernel(hisi_mdc_ion->ion_client, hisi_mdc_ion->ion_handle);
    ion_free(hisi_mdc_ion->ion_client, hisi_mdc_ion->ion_handle);
    ion_client_destroy(hisi_mdc_ion->ion_client);
    hisi_mdc_ion->ion_client = NULL;
    hisi_mdc_ion->ion_handle = NULL;
    hisi_mdc_ion->virt_addr  = NULL;
    kfree(hisi_mdc_ion);
...
static void set_shared_mdc_pa_addr(u64 mdc_pa_addr)
{
...
/*lint -save -e429*/
static ion_phys_addr_t get_mdc_addr_pa(size_t size, size_t align, unsigned int heap_id_mask, unsigned int flags)
{
    struct rproc_boot_device *dev = &rproc_boot_dev;
    struct hisi_isp_ion_s *hisi_mdc_ion = NULL;
    ion_phys_addr_t mdc_ion_phys = 0x0 ;
    size_t mdc_ion_size = 0;
    int ret = 0;

    hisi_mdc_ion = kzalloc(sizeof(*hisi_mdc_ion), GFP_KERNEL);/*lint !e838 */
    if (!hisi_mdc_ion) {
        pr_err("in %s hisi_mdc_ion kzalloc is failed\n", __func__);
        return 0;
    }

    hisi_mdc_ion->ion_client = hisi_ion_client_create("mdc_ion_client");
    if (IS_ERR(hisi_mdc_ion->ion_client)) {
        pr_err("hisi mdc_ion_client create failed \n");
        kfree(hisi_mdc_ion);
        return 0;
    }

    hisi_mdc_ion->ion_handle = ion_alloc(hisi_mdc_ion->ion_client, size, align, heap_id_mask, flags);
    if (IS_ERR(hisi_mdc_ion->ion_handle)){
        pr_err("hisi ion_alloc failed \n");
        ion_client_destroy(hisi_mdc_ion->ion_client);
        kfree(hisi_mdc_ion);
        return 0;
    }
    hisi_mdc_ion->virt_addr = ion_map_kernel(hisi_mdc_ion->ion_client, hisi_mdc_ion->ion_handle);
    if(hisi_mdc_ion->virt_addr == NULL)
    {
    ...
    ret = ion_phys(hisi_mdc_ion->ion_client, hisi_mdc_ion->ion_handle,&mdc_ion_phys, &mdc_ion_size);/*lint !e838 */
    if (ret < 0)
    {
											...
											int offset, int avail)
{
    ...
    align = 12;/*lint !e838 */
    heap_id_mask = (0x1 << 14);/*lint !e838 */
    flags = (0x1 << 4);/*lint !e838 */
    ...
    if (!strncmp(rsc->name, "ISP_MEM_BOOTWARE", strlen(rsc->name)))/*lint !e64 */
        ...
        addr = (u64)get_mdc_addr_pa(rsc->len, align, heap_id_mask, flags);/*lint !e747 */
        if (!addr)
        {
            dev_err(dev, "get_mdc_addr_pa NULL! \n");
            return -EINVAL;
        }
...
static struct hisi_rproc_data *hisi_rproc_data_dtget(struct device *pdev)
{
    ...
    dev->remap_addr =(u64)(unsigned)(ISP_BASEADDR + addr);
    RPROC_DEBUG("isp-remap-addr.0x%llx \n", dev->remap_addr);
