			    ...
			    struct pinctrl_gpio_range *range, unsigned pin)
{
	...
	list_for_each_safe(pos, tmp, &pcs->gpiofuncs) {
		...
		if (pin >= frange->offset + frange->npins
			...
			continue;
		mux_bytes = pcs->width / BITS_PER_BYTE;
		data = pcs->read(pcs->base + pin * mux_bytes) & ~pcs->fmask;/*lint !e679 */
		data |= frange->gpiofunc;
		pcs->write(data, pcs->base + pin * mux_bytes);/*lint !e679 */
		break;
	}
	return 0;
}
...
static void pcs_pinconf_clear_bias(struct pinctrl_dev *pctldev, unsigned pin)
{
	unsigned long config;
	int i;
	for (i = 0; i < ARRAY_SIZE(pcs_bias); i++) {/*lint !e574 */
				...
				unsigned pin, unsigned long *config)
{
	...
	for (i = 0; i < func->nconfs; i++) {/*lint !e574 */
		...
		case PIN_CONFIG_INPUT_SCHMITT:
			for (j = 0; j < func->nconfs; j++) {/*lint !e574 */
				...
				unsigned num_configs)
{
	...
	for (j = 0; j < num_configs; j++) {/*lint !e574 */
		for (i = 0; i < func->nconfs; i++) {/*lint !e574 */
...

{
	...
	for (i = 0; i < ARRAY_SIZE(prop2); i++) {/*lint !e574*/
		if (of_find_property(np, prop2[i].name, NULL))
			nconfs++;
	}
	for (i = 0; i < ARRAY_SIZE(prop4); i++) {/*lint !e574*/
