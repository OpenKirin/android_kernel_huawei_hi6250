		...
		struct map_result *result)
{
	...
	if (format->is_tile) {
		...
		if (phys_len < format->header_size ||
			!format->phys_page_line || !format->virt_page_line) {
			pr_err("[%s]wrong arg:phys_len 0x%lx, header_size 0x%lx\n",
				 __func__, phys_len, format->header_size);
			pr_err("[%s]wrong arg:phys_page_line 0x%lx, virt_page_line 0x%lx\n",
				 __func__, format->phys_page_line, format->virt_page_line);
			return -EINVAL;
		}
