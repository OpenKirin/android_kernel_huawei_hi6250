...
extern int read_gyro_temperature_offset_from_nv(void);
static void get_gyro_calibrate_data(void)
{
    ...
    if (0 == gyro_first_start_flag)
    {
        ...
        {
            ret=read_gyro_temperature_offset_from_nv();
            if(ret) hwlog_err( "get_gyro_temperature calibrate_data read from nv fail, ret=%d", ret);
            else
            {
