...
static int sar_sensor_i2c_detect(struct sar_sensor_detect sar_detect)
{
	...
	bus_num = sar_detect.bus_num;
	i2c_address = sar_detect.i2c_address;
	reg_add = sar_detect.chip_id;
	buf_temp[0] = 0;
	hwlog_info
		   ("In %s! bus_num = 0x%x, i2c_address = 0x%x, reg_add = 0x%x , chip_id_value = 0x%x 0x%x\n",
			      ...
			      char *buf)
{
	...
	hwlog_info
		   ("In %s! final_detect_result=%d, semtech_detect_result=%d, adi_detect_result=%d , cypress_detect_result=%d\n",
