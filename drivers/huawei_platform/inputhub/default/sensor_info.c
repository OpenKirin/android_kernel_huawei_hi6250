...
BH1745_ALS_PARA_TABLE als_para_diff_tp_color_table[] = {
	...
	{272,225,227,1264,1122,272,631,1899,100,-2580,9747,-4087,
...
APDS9251_ALS_PARA_TABLE apds_als_para_diff_tp_color_table[] = {
	...
	{ALPS, V1, DEFAULT_TPLCD, GOLD,
	 {30165, -1970, 1572, 1377, 1056, 1019, 728, 1019, 415, 171, 163,
	  2900, 3903, 1069, 110, 3705, 9149, 5000, 200, 1, 24} },
	{ALPS, V1, DEFAULT_TPLCD, BLACK,
	 {22462, -887, 1285, 1142, 967, 1010, 806, 1010, 400, 168, 162,
	 3445, 4599, 1466, 130, 3283, 8671, 5000, 200, 1, 29} },
	{ALPS, V1, DEFAULT_TPLCD, GRAY,
	 {22462, -887, 1285, 1142, 967, 1010, 806, 1010, 400, 168, 162,
	 3445, 4599, 1466, 130, 3283, 8671, 5000, 200, 1, 29} },
	{ALPS, V1, DEFAULT_TPLCD, WHITE,
	 {8136, 1274, 364, 328, 299, 1000,1062, 1000, 294, 123, 121,
	 9158, 14205, 6147, 376, 2872, 6735, 5000, 200, 1, 42} },
};
...
int read_gyro_temperature_offset_from_nv(void)
{
	int ret = 0;
	struct hisi_nve_info_user user_info;
	write_info_t pkg_ap;
	...
	memset(&pkg_mcu, 0, sizeof(pkg_mcu));
	user_info.nv_operation = NV_READ_TAG;
	user_info.nv_number = GYRO_TEMP_CALI_NV_NUM;
	user_info.valid_size = GYRO_TEMP_CALI_NV_SIZE;
	strncpy(user_info.nv_name, "GYTMP", sizeof(user_info.nv_name));
	user_info.nv_name[sizeof(user_info.nv_name) - 1] = '\0';
	ret = hisi_nve_direct_access(&user_info);
	if (ret != 0) {
		hwlog_err("nve_direct_access read error(%d)\n", ret);
	...
	pkg_ap.tag = TAG_GYRO;
	pkg_ap.cmd = CMD_GYRO_TMP_OFFSET_REQ;
	pkg_ap.wr_buf = gyro_temperature_offset;
	pkg_ap.wr_len = sizeof(gyro_temperature_offset);
	ret = write_customize_cmd(&pkg_ap, &pkg_mcu);
	memcpy(&gyro_temperature_calibrate_data, pkg_ap.wr_buf,
	       (sizeof(gyro_temperature_calibrate_data) <
		pkg_ap.wr_len) ? sizeof(gyro_temperature_calibrate_data) :
		pkg_ap.wr_len);
	if (ret) {
		hwlog_err("send gyro temperature offset data to mcu fail,ret=%d\n",
			  ret);
		return -1;
	}
	if (pkg_mcu.errno != 0) {
		hwlog_err("set gyro temperature offset fail,err=%d\n",
			  pkg_mcu.errno);
		__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__,
			       "set gyro temperature offset fail\n");
	} else {
		hwlog_info("send gyro temperature offset data to mcu success\n");
	}
	return 0;
}
...
int write_gyro_temperature_offset_to_nv(char *temp, int length)
{
	...
	if (temp == NULL) {
		hwlog_err
		    ("write_gyro_temp_offset_to_nv fail, invalid para!\n");
		__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__,
			"write_gyro_temp_offset_to_nv fail, invalid para!\n");
		return -1;
	}
	...
	hwlog_err("write_gyro_temp_offset_to_nv suc len=%d \n", length);
	return ret;
}
...
void reset_calibrate_when_recovery_iom3(void)
{
	...
	if (strlen(sensor_chip_info[GYRO])) {
		write_customize_cmd_noresp(TAG_GYRO, CMD_GYRO_OFFSET_REQ,
					   ...
					   sizeof(int) * 15);
		write_customize_cmd_noresp(TAG_GYRO, CMD_GYRO_TMP_OFFSET_REQ,
					   &gyro_temperature_calibrate_data,
					   GYRO_TEMP_CALI_NV_SIZE);
	}
