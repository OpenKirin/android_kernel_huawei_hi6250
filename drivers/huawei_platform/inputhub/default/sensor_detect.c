...
static void read_charger_data_from_dts(struct device_node *dn)
{
	...
	if(!gpio_is_valid(temp))
	    ...
	    charger_dts_data.gpio_cd = (GPIO_NUM_TYPE)temp;

	temp = of_get_named_gpio(dn,"gpio_cd_sh",0);
	if(!gpio_is_valid(temp))
	    hwlog_err("gpio_cd_sh is not valid\n");
	else
	    charger_dts_data.gpio_cd_sh = (GPIO_NUM_TYPE)temp;

	temp = of_get_named_gpio(dn,"gpio_int",0);
	if(!gpio_is_valid(temp))
	    hwlog_err("gpio_int is not valid\n");
	else
	    charger_dts_data.gpio_int = (GPIO_NUM_TYPE)temp;

	temp = of_get_named_gpio(dn,"gpio_int_sh",0);
	if(!gpio_is_valid(temp))
	    hwlog_err("gpio_int_sh is not valid\n");
	else
	    charger_dts_data.gpio_int_sh = (GPIO_NUM_TYPE)temp;
...
int init_sensors_cfg_data_from_dts(DETECT_MODE mode)
{
	...
	for_each_node_with_property(dn, "sensor_type") {
		...
		if (ret) {
			...
			if(CAP_PROX == s_detect_manager[index].sensor_id){
				if (of_property_read_u32(dn, "bus_number", &i2c_bus_num)
					   || of_property_read_u32(dn, "reg", &i2c_address)
					    || of_property_read_u32(dn, "chip_id_register", &register_add)) {
					hwlog_err
						   ("sar sensor :read i2c bus_number (%d)or bus address(%x) or chip_id_register(%x) from dts fail\n",
						    i2c_bus_num, i2c_address, register_add);
				}

				if (of_property_read_u32_array(dn, "chip_id_value", wia, 2)) {
					hwlog_err
						   ("sar sensor:read chip_id_value (id0=0x%x id1=0x%x) from dts fail len=%d\n",wia[0],wia[1]);
				}

				hwlog_info
					   ("sar sensor:bus_number (%d) slave address(0x%x) chip_id_register(0x%x) chipid value 0x%x 0x%x\n",
				...
				if (!strncmp(sensor_chip_info[CAP_PROX], "huawei,semtech-sx9323",
					...
					semtech_sar_detect.bus_num = (uint8_t)i2c_bus_num;
					semtech_sar_detect.i2c_address = (uint8_t)i2c_address;
					...
					adi_sar_detect.bus_num = (uint8_t)i2c_bus_num;
					adi_sar_detect.i2c_address = (uint8_t)i2c_address;
				...
				}else if (!strncmp(sensor_chip_info[CAP_PROX], "huawei,cypress_sar_psoc4000",
					strlen("huawei,cypress_sar_psoc4000"))){
					hwlog_info("sar sensor from dts is cypress_sar_psoc4000\n");
					cypress_sar_detect.detect_flag= 1;
					cypress_sar_detect.bus_num = (uint8_t)i2c_bus_num;
					cypress_sar_detect.i2c_address = (uint8_t)i2c_address;
...
static int gyro_temperature_offset_from_mcu(const pkt_header_t *head)
{
	return write_gyro_temperature_offset_to_nv(((const
	...
	switch (s_id){
		case GYRO:
			register_mcu_event_notifier(TAG_GYRO, CMD_GYRO_SELFCALI_REQ, gyro_calibrate_data_from_mcu);
                    register_mcu_event_notifier(TAG_GYRO, CMD_GYRO_TMP_OFFSET_REQ, gyro_temperature_offset_from_mcu);
			break;
		case MAG:
