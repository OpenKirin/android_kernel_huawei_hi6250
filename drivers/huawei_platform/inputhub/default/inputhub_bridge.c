...
void restart_iom3(void)
{
    ...
    ret =
        RPROC_ASYNC_SEND(ipc_ap_to_lpm_mbx, (mbox_msg_t*)&reset_msg,
                         1);
    ...
    ret =
        RPROC_ASYNC_SEND(ipc_ap_to_iom_mbx, (mbox_msg_t*)&boot_iom3,
                         1);
...
int inputhub_mcu_send(const char* buf, unsigned int length)
{
    ...
    ret =
        RPROC_SYNC_SEND(ipc_ap_to_iom_mbx, (mbox_msg_t*) buf, len,
                        NULL, 0);
...
recovery_iom3:
    ...
    rc = RPROC_SYNC_SEND(ipc_ap_to_iom_mbx, &tx_buffer, 1, &ack_buffer,
                         1);
...
void check_ipc_mbx_from_dts(void)
{
    ...
    sh_node =
        of_find_compatible_node(NULL, NULL, "huawei,sensorhub_ipc_cfg");

    if (!sh_node) {
        hwlog_err("%s, can not find node huawei,sensorhub_ipc_cfg",
                  __func__);
        return;
    }
