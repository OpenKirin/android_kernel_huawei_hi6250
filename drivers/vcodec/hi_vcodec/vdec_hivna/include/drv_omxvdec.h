...
#else
#define NATIVE_ALIGN_STEP               (64) //stride align step when native buffer enable
#endif
...
typedef struct hi_OMXVDEC_SEQ_INFO {
	...
	HI_U32 disp_height;
	HI_U32 stride;
	HI_U32 hfbc_header_y_stride;
	HI_U32 hfbc_payload_y_stride;
	HI_U32 hfbc_header_uv_stride;
	HI_U32 hfbc_payload_uv_stride;
	HI_U32 frame_size;
...
typedef struct hi_OMXVDEC_HFBC_INFO {
	HI_U32 hfbc_header_y_addr;
	HI_U32 hfbc_header_y_stride;
	HI_U32 hfbc_payload_y_addr;
	HI_U32 hfbc_payload_y_stride;
	HI_U32 hfbc_header_uv_addr;
	HI_U32 hfbc_header_uv_stride;
	HI_U32 hfbc_payload_uv_addr;
	HI_U32 hfbc_payload_uv_stride;
	HI_U32 hfbc_scramble;
} OMXVDEC_HFBC_INFO;

typedef struct{
	HI_U32 flag;    //0: has not metadata; 1: has metadata;
	HI_U32 display_primaries_x[3];
	HI_U32 display_primaries_y[3];
...
typedef struct hi_OMXVDEC_BUF_DESC {
	...
	HI_U32             is_hfbc;
	OMXVDEC_HFBC_INFO  hfbc_info;
	OMXVDEC_METADATA_S metadata;
	HI_U64             kerneladdr;
	HI_VOID           *bufferaddr;
	HI_VOID           *client_data;
} OMXVDEC_BUF_DESC;
...
typedef struct {
	...
	HI_U32 msgcode;
	HI_U32 msgdatasize;
	CMD_OMXVDEC_MSG_DATA dat;
} CMD_OMXVDEC_MSG_INFO;
...
#define VDEC_IOCTL_GET_DTS_CONFIG      \
