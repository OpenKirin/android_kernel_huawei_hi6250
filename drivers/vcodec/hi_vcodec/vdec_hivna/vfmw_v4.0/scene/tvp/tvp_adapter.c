...
SINT32 TVP_VDEC_SecureInit(VOID)
{
	...
	SecPrint(PRN_ALWS, "%s Enter\n", __func__);
	if (g_SecInit != 0) {
		SecPrint(PRN_ALWS, "%s, TVP_VDEC_SecureInit init already\n", __func__);
		return VDEC_OK;
	}
...
SINT32 TVP_VDEC_SecureExit(VOID)
{
	...
	if (!g_SecInit) {
		SecPrint(PRN_ALWS, "FATAL %s, init first\n", __func__);
		return VDEC_ERR;
	}
