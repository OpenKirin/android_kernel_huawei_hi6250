...
SINT32 MEM_AddMemRecord(UADDR PhyAddr, VOID *VirAddr, UINT32 Length)
{
	...
	for (i = 0; i < MAX_MEM_MAN_RECORD_NUM; i++) {
		if ((s_MemRecord[i].PhyAddr <= PhyAddr) && (PhyAddr < s_MemRecord[i].PhyAddr + s_MemRecord[i].Length)) {
			dprint(PRN_ALWS, "%s ERROR: Phy [0x%x ~ 0x%x] overlap with Record[%d] [0x%x ~ 0x%x]\n",
				__func__, PhyAddr, PhyAddr + Length, i, s_MemRecord[i].PhyAddr, s_MemRecord[i].PhyAddr + s_MemRecord[i].Length);
			IsErrorFlag = 1;
			break;
		}
...
VOID MEM_DumpRecord(VOID)
{
	...
	for (i = 0; i < MAX_MEM_MAN_RECORD_NUM; i++) {
		if (s_MemRecord[i].Length == 0)
			continue;
		dprint(PRN_ALWS, " %d: PhyAddr %x, VirAddr %pK, Length %d, IsSecure %d\n", i, s_MemRecord[i].PhyAddr,
			s_MemRecord[i].VirAddr, s_MemRecord[i].Length, s_MemRecord[i].IsSecMem);
	}
	dprint(PRN_ALWS, "-------------------------- END --------------------------\n\n");
