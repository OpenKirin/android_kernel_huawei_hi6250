...
typedef struct {
	...
	UINT32  HeadYPhyAddr[MAX_FRAME_NUM];
	UINT32  HeadCPhyAddr[MAX_FRAME_NUM];
	UINT32  HeadStride;
	UINT32  cfginfo_msg_addr;
	UINT32  uvstride_1d;
...
#define HEVC_APC_SIZE                       		 (16)

#define HEVC_STREAM_BASE_ALIGN                       (15)
...
#define HEVC_PPFD_BUFFER_LEN                         (64*4*800)
...
typedef struct {
	...
	UADDR       StreamPhyAddr[2];         //g_BitsObj.phy_addr&(0xFFFFFFF0) Ș ptr_strm_para->StreamPack[1].PhyAddr &(0xFFFFFFF0)
	UINT32      StreamBitOffset[2];
	SINT32      StreamLength[2];
...
#define VC1VFMW_MAX_SLC_NUM    256
#define VC1VFMW_MAX_PIC_WIDTH  128
#define VC1VFMW_MAX_PIC_HEIGHT 128
...
#endif
#define  VP9_ONE_MSG_SLOT_LEN                     (64*4*5)
#define  VP9_APC_SIZE                       	  (16)  //< max. number of apc size
...
typedef struct {
	...
	UINT32  ImgPhyAddr[MAX_FRAME_NUM];  // for hfbc
	UINT32  HeadYPhyAddr[MAX_FRAME_NUM];
	UINT32  HeadCPhyAddr[MAX_FRAME_NUM];
	...
	UINT32  imgstride;
	UINT32  HeadStride;
	UINT32 LastHeadStride;
	UINT32 GoldenHeadStride;
	UINT32 AltHeadStride;
	UINT32  Apc2Idc[VP9_APC_SIZE];
	UINT32  ApcSize;
	...
	UINT32 vpss_info_addr;
	UINT32 vpss_info_stride;
	UINT32 CurrPmvAddr;
	UINT32 SegIdAddr;
	UINT32 SegIdStride;
	SINT32 Compress_en;
	SINT32 mbinfo_offset;
