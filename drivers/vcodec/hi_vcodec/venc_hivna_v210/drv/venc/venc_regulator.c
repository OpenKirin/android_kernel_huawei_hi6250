...
static HI_S32 Venc_GetDtsConfigInfo(struct platform_device *pdev, VeduEfl_DTS_CONFIG_S *pDtsConfig)
{
	...
	HI_DBG_VENC("%s, IsFPGA : %x, VeduIrqNumNorm : %x, VeduIrqNumProt : %x, VeduIrqNumSafe : %x,\
		VencRegBaseAddr : %x, VencRegRange : %x\n", __func__, pDtsConfig->IsFPGA,\
		pDtsConfig->VeduIrqNumNorm,	pDtsConfig->VeduIrqNumProt, pDtsConfig->VeduIrqNumSafe,\
		pDtsConfig->VencRegBaseAddr, pDtsConfig->VencRegRange);
...
static HI_S32 Venc_Regulator_Enable_Kirin660(HI_VOID)
{
	HI_S32 ret = HI_FAILURE;
...
static HI_S32 Venc_Regulator_Disable_Kirin660(HI_VOID)
{
	HI_S32 ret = HI_FAILURE;
	HI_DBG_VENC("%s, Venc_Regulator_Disable g_VencRegulator:0x%x\n", __func__, g_VencRegulator);
...
#endif
#ifdef PLATFORM_KIRIN660
	return Venc_Regulator_Enable_Kirin660();
#endif
...
#endif
#ifdef PLATFORM_KIRIN660
	return Venc_Regulator_Disable_Kirin660();
#endif
