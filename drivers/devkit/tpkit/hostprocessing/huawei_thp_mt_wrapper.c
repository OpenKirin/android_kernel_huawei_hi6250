					...
					unsigned long event, void *data)
{
	...
	struct thp_mt_wrapper_data *thp_mt_wrapper =
		container_of(self, struct thp_mt_wrapper_data, fb_notif);
	...
	if (event == FB_EVENT_BLANK && blank == FB_BLANK_UNBLANK) {
		...
		kobject_uevent(&thp_mt_wrapper->input_dev->dev.kobj,
					KOBJ_ONLINE);
	} else if (event == FB_EARLY_EVENT_BLANK && blank == FB_BLANK_POWERDOWN) {
		THP_LOG_INFO("%s:input_mt_wrapper early suspend\n", __func__);
...
static int thp_parse_input_config(struct thp_input_dev_config *config)
{
	...
	if (!thp_dev_node) {
		THP_LOG_INFO("%s:not found node %s, use defatle config\n",
					__func__);
		goto use_defaule;
	}
...
int thp_mt_wrapper_init(void)
{
	...
	input_set_abs_params(input_dev, ABS_MT_TOOL_TYPE,
			0, mt_wrapper->input_dev_config.tool_type_max, 0, 0);
...
#if defined(CONFIG_FB)
	mt_wrapper->fb_notif.notifier_call = fb_notifier_callback;
	rc = fb_register_client(&mt_wrapper->fb_notif);
	if (rc) {
		THP_LOG_ERR("%s: register fb_notifier fail\n", __func__);
		goto fb_notify_reg_err;
	}
