...
static int thp_novatech_init(struct thp_device *tdev)
{
	...
	rc = thp_nvt_check_spi_master_capacity(cd->sdev);
	if (rc) {
	...
	rc = thp_parse_timing_config(nova_node, cd);
	if (rc)
...
static int thp_novatech_chip_detect(struct thp_device *tdev)
{
	...
	int rc = thp_nvt_ts_check_chip_ver_trim(tdev->thp_core->sdev);
	if (rc) {
...
{
	unsigned char get_frame_cmd = 0x20;
...
{
	THP_LOG_INFO("%s: called\n", __func__);
	gpio_set_value(tdev->thp_core->rst_gpio, 1);
	gpio_set_value(tdev->thp_core->cs_gpio, 1);
	thp_do_time_delay(tdev->thp_core->timing_config.resume_reset_after_delay_ms);
...
{
	THP_LOG_INFO("%s: called\n", __func__);
	gpio_set_value(tdev->thp_core->rst_gpio, 0);
	gpio_set_value(tdev->thp_core->cs_gpio, 0);
	thp_do_time_delay(tdev->thp_core->timing_config.suspend_reset_after_delay_ms);
