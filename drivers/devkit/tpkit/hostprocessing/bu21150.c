                                      ...
                                      struct kobj_attribute* attr, char* buf)
{
    ...
    return snprintf(buf, PAGE_SIZE, "%s-%s-jdihost\n", ts->ic_name,
                        ts->project_id);
}
...
static int htp_init_chip_info(struct bu21150_data* ts)
{
	int ret = hostprocessing_get_project_id(ts->project_id);
	if(ret)
		HOST_LOG_ERR("%s:get project id form LCD fail\n", __func__);
	else
		HOST_LOG_INFO("%s:get project id form LCD:%s\n", __func__, ts->project_id);

	ts->project_id[THP_PROJECT_ID_LEN - 1] = '\0';
...
static void copy_frame(struct bu21150_data* ts)
{
    ...
    if (ts->frame_count != pre_frame_count)
        HOST_LOG_ERR("%s:ts frame buferr full start\n", __func__);

        tmp = list_first_entry(&ts->frame_list.list,
                               struct bu21150_frame, list);
        list_del(&tmp->list);
        kfree(tmp);
        pre_frame_count = ts->frame_count;
        ts->frame_count--;
    } else if(pre_frame_count >= BU21150_LIST_MAX_FRAMES) {
        HOST_LOG_ERR("%s:ts frame buferr full exception has been restored\n", __func__);
        pre_frame_count = ts->frame_count;
    }
...
static int parse_dtsi(struct device* dev, struct bu21150_data* ts)
{
    ...
    retval = of_property_read_string(np, "ic_name", &ts->ic_name);
    if (retval) {
        ts->ic_name = "unknown";
        HOST_LOG_ERR("get ic_name failed, set default\n");
    } else {
	HOST_LOG_ERR("ic_name : %s\n", ts->ic_name);
    }
