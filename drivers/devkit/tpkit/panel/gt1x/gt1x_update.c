...
int gt1x_update_firmware(void)
{
...
#if defined (CONFIG_HUAWEI_DSM)
		gt1x_ts->dev_data->ts_platform_data->dsm_info.constraints_UPDATE_status =  GT1X_CHECK_UPDATE_FILE_FAIL;
#endif
		goto update_exit;
	}
	...
	if (ret) {
		update_info.status = UPDATE_STATUS_ABORT;
#if defined (CONFIG_HUAWEI_DSM)
		gt1x_ts->dev_data->ts_platform_data->dsm_info.constraints_UPDATE_status =  GT1X_UPDATE_JUDGE_FAIL;
#endif
		goto  update_exit;
	}
	...
	if( subsystem_info==NULL ) {
		...
		update_info.status = UPDATE_STATUS_ABORT;
#if defined (CONFIG_HUAWEI_DSM)
		gt1x_ts->dev_data->ts_platform_data->dsm_info.constraints_UPDATE_status =  GT1X_GET_ISP_FAIL;
#endif
		goto  update_exit;
	}
...
#if defined (CONFIG_HUAWEI_DSM)
		gt1x_ts->dev_data->ts_platform_data->dsm_info.constraints_UPDATE_status =  GT1X_RUN_ISP_FAIL;
#endif
		goto  update_exit;
	}
...
#if defined (CONFIG_HUAWEI_DSM)
			gt1x_ts->dev_data->ts_platform_data->dsm_info.constraints_UPDATE_status =  GT1X_BURN_SUBSYSTEM_FAIL;
#endif
			goto  update_exit;
		}
	...
	if (ret) {
		...
		if (update_info.status == UPDATE_STATUS_ABORT) {
			...
			if (!dsm_client_ocuppy(ts_dclient)) {
				TS_LOG_INFO("%s: try to client record DSM_TP_FWUPDATE_ERROR_NO(%d)\n", __func__, DSM_TP_FWUPDATE_ERROR_NO);
				dsm_client_record(ts_dclient, "fw update result: failed updata_status is %d.\n",
					gt1x_ts->dev_data->ts_platform_data->dsm_info.constraints_UPDATE_status);
				dsm_client_notify(ts_dclient,DSM_TP_FWUPDATE_ERROR_NO);
			}
			strncpy(gt1x_ts->dev_data->ts_platform_data->dsm_info.fw_update_result, "failed", strlen("failed"));
#endif
