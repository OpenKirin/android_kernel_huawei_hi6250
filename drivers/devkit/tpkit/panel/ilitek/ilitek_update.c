...
#define OUTWRIE_BUF 64
#define OUTWRIE_ADRR_LEN  4
int outwrite(unsigned int address, unsigned int data, int size)
{
	int ret =0;
	int i = 0;
	char outbuff[OUTWRIE_BUF] ={0};
	outbuff[0] = REG_START_DATA;
	outbuff[1] = (char)((address & DATA_SHIFT_0) >> 0);
	outbuff[2] = (char)((address & DATA_SHIFT_8) >> 8);
	outbuff[3] = (char)((address & DATA_SHIFT_16) >> 16);
	if((size + OUTWRIE_ADRR_LEN ) > OUTWRIE_BUF)
	{
		TS_LOG_ERR("%s:size is too large,please check it.", __func__);
		return TRANSMIT_ERROR;
	}
	for(i = 0; i < size; i++)
	{
		outbuff[i + OUTWRIE_ADRR_LEN] = (char)(data >> (8 * i));
	}
	ret = ilitek_i2c_write( outbuff, size + OUTWRIE_ADRR_LEN);
	if (ret ) {
		TS_LOG_ERR("%s:fai", __func__);
		return TRANSMIT_ERROR;
	}
...
Retry:
		...
		ret = ilitek_i2c_write( buf, FW_UPDATE_BUFF_LEN);
		if (ret) {
			TS_LOG_ERR("%s, line = %d TRANSMIT_ERROR\n", __func__, __LINE__);
			goto Retry;
		}
