...
void dump_smmu_regs(struct fd_device *devp)
{
    int i ;
    if(devp->smmu_vir_base!=NULL)
        {
            //fd_info("devp->smmu_vir_base = 0x%x", devp->smmu_vir_base);
            for (i = 0 ; i < 0x500;)
            {
                //fd_info("smmu:offset:0x%x,  reg = 0x%x", i, readl(devp->smmu_vir_base + i));
                i += 4;
            }
        }
}

void dump_master_regs(struct fd_device *devp)
{
    int i ;
    if(devp->mstr_vir_base!=NULL)
        {
            //fd_info("devp->mstr_vir_base = 0x%x", devp->mstr_vir_base);
            for (i = 0 ; i < 0x500;)
            {
                //fd_info("master:offset:0x%x, reg = 0x%x",i, readl(devp->mstr_vir_base + i));
                i += 4;
            }
        }
}
...
#if 0
static int fd_setup_rc(struct device_node *pnode,struct fd_device *fd_Dev)
{
    int i = 0, count = 0;
    const char *name =NULL;

    count = of_property_count_strings(pnode, "fd-pwr-seq");
    for (i=1; i<(count+1); i++) {
        of_property_read_string_index(pnode, "fd-pwr-seq", i-1, &name);
        if (!strncmp(name, "fd_regulator", strlen("fd_regulator"))) {
            fd_Dev->rc_seq[i] = FD_RC_REGULATOR;

        } else if (!strncmp(name, "fd_clk", strlen("fd_clk"))) {
            fd_Dev->rc_seq[i] = FD_RC_CLK;

        } else {
            fd_err("no such name:%s", name);
        }
    }

    fd_Dev->rc_seq[0] = count;
...
static long AHFD_Get_Version(struct fd_device *pdev, unsigned int cmd, unsigned long arg)
{
    ...
    if (__put_user(version, (const uint32_t __user *)arg))
    {
...
static long AHFD_Write_Regs(struct fd_device *pdev, unsigned int cmd, unsigned long arg)
{
    ...
    {
        fd_err("AHFD_WRITE_REGS !access_ok");
        rc = -EFAULT;
        goto _write_exit;
    }
    ...
    if (copy_from_user(pdev->registerBufferWrite, ((struct fd_reg_array*)arg)->values, regs.length * sizeof(uint32_t)))
    {
    ...
    if(NULL != pdev->fd_viraddr)
    {
        ...
        uint32_t *regStart = pdev->registerBufferWrite, used = 0;
        uint32_t regVal;

        while(used < regs.length)
        {
            offset = *(regStart++);
            size   = *(regStart++);
            addr   = base_addr + offset;

            if (offset + 4 * size > AHFD_MAX_OFFSET)
            {
                fd_err("AHFD_WRITE_REGS illegal write operation");
                rc = -EACCES;
                goto _write_exit;
            }
...
static long AHFD_Read_Regs(struct fd_device *pdev, unsigned int cmd, unsigned long arg)
{
    ...
    if (copy_from_user(pdev->registerBufferRead, ((struct fd_reg_array*)arg)->values, regs.length * sizeof(uint32_t)))
    {
    ...
    if(NULL != pdev->fd_viraddr)
    {
        ...
        char __iomem *addr;
        uint32_t size, k, offset;
        uint32_t *regStart = pdev->registerBufferRead, used = 0;
        uint32_t regVal;
        while(used < regs.length)
        {
            offset = *(regStart++);
            ...
            if (offset + 4 * size > AHFD_MAX_OFFSET)
            {
                fd_err("AHFD_READ_REGS illegal read operation");
                rc = -EACCES;
                goto _read_exit;
            }
    ...
    if (copy_to_user((uint32_t*)((struct fd_reg_array*)arg)->values, pdev->registerBufferRead, regs.length * sizeof(uint32_t)))
    {
...
static long AHFD_Wait_Result(struct fd_device *pdev, unsigned long arg)
{
    long rc = 0;
    unsigned int waitType = (unsigned int)arg;
    ...
    if(waitType & RESULT_TYPE_TME)
    {
        ...
        TME_time = ktime_to_us(ktime_sub(tme_end, tme_start));
...
static long AHFD_Set_Mode(struct fd_device *pdev, unsigned long arg)
{
    int mode = (int)arg;
...
static long fd_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        case AHFD_WAIT_RESULT:
            {
                rc = AHFD_Wait_Result(pdev, arg);
                break;
            }
        case AHFD_SET_MODE:
            {
                rc =  AHFD_Set_Mode(pdev, arg);
                break;
            }
...
static void fd_get_pgd_base(struct fd_device *fd_Dev)
{
    ...
    fd_info("[%s] info.iova.(0x%lx, 0x%lx) phy_pgd_base.0x%x\n", __func__,
            info->iova_start,
            info->iova_size,
            fd_Dev->phy_pgd_base);
...
//#define MAX_SMMU_OFFSET_LIST_NUM 20
static int fd_get_offset_list(void)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        //fd_info("smmu_offset_list[%d] = 0x%x\n",i,smmu_offset_list[i]);
    }
    ...
    for (i = 0; i < len; i++) {
        ...
        //fd_info("mstr_offset_list[%d] = 0x%x\n",i,mstr_offset_list[i]);
    }
...
MODULE_AUTHOR("x00380280 <xuefeng15@huawei.com>");
/*lint -e753 +esym(753,*)*/
