...
typedef struct _acs_light_weight_t
{
    HI_U16 rg_val;
    HI_U16 bg_val;
}acs_light_weight_t;
...
{
    char           sharpen_idx;
    unsigned char  SharpSel;
    unsigned short SharpAmt1;
    unsigned short EdgeAmt;
    unsigned char  Reserved_0;
    ...
    unsigned char  Reserved_25;//32
} sharpen_exif_t;//32 byte

typedef struct _de_exif_t{
    //add exif info
    unsigned int de_filter_idx;
    unsigned int globalgainHF;
    unsigned int globalgainMF;
    unsigned int globalgainLF;
    int lutMFNoise3x3LPF[4];
    int lutMFNoise5x5LPF[3];
    int lutLFNoise[3];
    unsigned int minGainScale;
    unsigned int gainScale_luma_y0;
    unsigned int gainScale_luma_y2;
    unsigned int blr_luma_y0;
    unsigned int reserved;
} de_exif_t;
...
typedef struct _lsctop_to_lsc_t
{
    lsc_gain_intensity_t    gain_intensity;//32
    // grid_info_t             grid_info;
    mod_mrs_size_profile_t mrs_size_profile_fromcalib;//25360
    unsigned int                     calib_image_width;
    unsigned int                     calib_image_height;
} lsctop_to_lsc_t;//25368bytes
