...
static int support_dual_tempeture = 1;  //1-enable 0-disable
...
int data)
{
    ...
    i2c_func->i2c_write(i2c_client, REG_L2_TOR, MAX_FLASH_TORCH_CURRENT - level);
    i2c_func->i2c_read(i2c_client, REG_MODE, &reg_val);
...
struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    rc = hw_mp3336_param_check((char *)buf, &param, 1);
    if (rc < 0) {
        cam_err("%s failed to check param.", __func__);
        return rc;
    }
    support_dual_tempeture = (int)param;
    cam_info("%s support_dual_tempeture = %d", __func__, support_dual_tempeture);
