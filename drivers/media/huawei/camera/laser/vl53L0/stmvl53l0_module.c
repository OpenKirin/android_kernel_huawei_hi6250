...
extern int stmvl53l0_match_id(struct stmvl53l0_data * data);
...
static int stmvl53l0_status(struct stmvl53l0_data* data, hwlaser_status_t *laser_status)
{
    ...
    if(NULL == data || NULL == laser_status)
        return rc;
    laser_status->status = stmvl53l0_match_id(data);
    if(laser_status->status == 1){
        strncpy(laser_status->name, "vl53l0_970", HWLASER_NAME_SIZE-1);
    }
...
extern int stmvl53l0_power_on_off(struct stmvl53l0_data *data, bool is_on);
long stmvl53l0_laser_ioctl(void *hw_data, unsigned int cmd, void  *p)
{
    ...
    switch (cmd) {
        case HWLASER_IOCTL_GET_INFO:
            ...
            strncpy(pinfo->name, "vl53l0_970", HWLASER_NAME_SIZE-1);
...
static int stmvl53l0_init_client(struct stmvl53l0_data *data)
{
    ...
    if (Status == VL53L0_ERROR_NONE) {
        ...
        pr_info("ProductRevisionMajor : %d\n", DeviceInfo.ProductRevisionMajor);
        pr_info("ProductRevisionMinor : %d\n", DeviceInfo.ProductRevisionMinor);
    }
