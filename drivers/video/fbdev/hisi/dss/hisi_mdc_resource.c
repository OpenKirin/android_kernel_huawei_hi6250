...
int hisi_mdc_resource_init(struct hisi_fb_data_type *hisifd, unsigned int platform)
{
	...
	switch (platform) {
		...
		case FB_ACCEL_KIRIN970:
			...
			// cppcheck-suppress *
#if 0 //mediacommon block don't release for using conpybit func.
			mdc_ops->mdc_channel[2].cap_available = CAP_BASE | CAP_DIM \
				| CAP_YUV_PACKAGE | CAP_YUV_SEMI_PLANAR | CAP_YUV_PLANAR \
				| CAP_YUV_DEINTERLACE;
			mdc_ops->mdc_channel[2].rch_idx = MDC_RCHN_V;
			mdc_ops->mdc_channel[2].wch_idx = MDC_WCHN_W;
			mdc_ops->mdc_channel[2].ovl_idx = MDC_OVL;
			mdc_ops->mdc_channel[2].wb_composer_type = DSS_WB_COMPOSE_MEDIACOMMON;
			mdc_ops->mdc_channel[2].ref_count = 0;
			mdc_ops->mdc_channel[2].status = FREE;
#endif
			break;
