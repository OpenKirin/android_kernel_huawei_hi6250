...
static void mipi_config_dphy_spec_parameter(char __iomem *mipi_dsi_base, struct hisi_panel_info *pinfo)
{
	...
	#if defined(CONFIG_HISI_FB_3650) || defined(CONFIG_HISI_FB_6250) || defined(CONFIG_HISI_FB_3660) || defined(CONFIG_HISI_FB_660)
...
static void mipi_init(struct hisi_fb_data_type *hisifd, char __iomem *mipi_dsi_base)
{
	...
	#if defined(CONFIG_HISI_FB_3660) || defined(CONFIG_HISI_FB_660)
...
/*lint -e712*/
	...
	if (is_mipi_cmd_panel(hisifd)) {
		...
		if (g_dss_version_tag == FB_ACCEL_KIRIN970_ES) {
			...
			twakeup_cnt = hisifd->dss_clk_rate.dss_pclk_dss_rate / 1000 * 3 / 2 / twakeup_clk_div;
		} else {
			twakeup_clk_div = 8;
...
#if defined(CONFIG_HISI_FB_970)
	//enable auto_ulps
	/*!(pinfo->vsync_ctrl_type & VSYNC_CTRL_MIPI_ULPS)*/
	if ((g_dss_version_tag == FB_ACCEL_KIRIN970_ES) && is_mipi_cmd_panel(hisifd)) {
...
#if defined(CONFIG_HISI_FB_970) || defined(CONFIG_HISI_FB_660)
	//bit13 lock sel enable (dual_mipi_panel bit29 set 1) ,colse clock gate
	...
	char __iomem *mipi_dsi_base, struct mipi_dsi_phy_ctrl* phy_ctrl)
{
	...
	//physical configuration PLL III
	mipi_config_phy_test_code(mipi_dsi_base, 0x00010016, (pinfo->dsi_phy_ctrl.rg_cp << 5));
#else
	// physical configuration PLL I
...
#elif defined(CONFIG_HISI_FB_3660) || defined(CONFIG_HISI_FB_660)
	set_reg(mipi_dsi_base + MIPIDSI_PHY_TMR_RD_CFG_OFFSET, 0x7FFF, 15, 0);
	set_reg(mipi_dsi_base + MIPIDSI_PHY_TMR_CFG_OFFSET,
...
int mipi_dsi_bit_clk_upt_isr_handler(struct hisi_fb_data_type *hisifd)
{
	...
	if ((g_dss_version_tag == FB_ACCEL_KIRIN970) || (g_dss_version_tag == FB_ACCEL_KIRIN970_ES) ||
		(g_dss_version_tag == FB_ACCEL_KIRIN660)) {
