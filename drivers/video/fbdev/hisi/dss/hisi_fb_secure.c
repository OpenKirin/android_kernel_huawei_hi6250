...
int configure_hdcp_service_security(unsigned int master_op_type, unsigned int enable)
{
	int ret = 0;
	...
	ret = atfd_hisi_service_access_register_smc(ACCESS_REGISTER_FN_MAIN_ID_HDCP,
			master_op_type, enable, (u64)ACCESS_REGISTER_FN_SUB_ID_HDCP_CTRL);
...
int configure_hdcp_int_security(void)
{
	int ret = 0;
	HISI_FB_DEBUG("+.\n");

	ret = atfd_hisi_service_access_register_smc(ACCESS_REGISTER_FN_MAIN_ID_HDCP,
			0x00, 0x00, (u64)ACCESS_REGISTER_FN_SUB_ID_HDCP_INT);

	HISI_FB_DEBUG("-.\n");
	return ret;
}
EXPORT_SYMBOL_GPL(configure_hdcp_int_security);
