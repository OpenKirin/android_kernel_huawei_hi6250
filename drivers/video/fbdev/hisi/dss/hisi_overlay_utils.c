	...
	dss_layer_t *layer, dss_rect_t *aligned_rect, bool rdma_stretch_enable)
{
	...
	chn_idx_temp = DSS_RCHN_V1; /*miami v1 surport vscfh*/
#else
	...
	if (pinfo->acm_support) {
		if (SCENE_MODE_GALLERY == hisifd->user_scene_mode || SCENE_MODE_DEFAULT == hisifd->user_scene_mode) {
			if (pinfo->acm_lut_hue_table_len > 0 && pinfo->acm_lut_hue_table
				...
				local_acm_lut_hue_table = cur_acm_reg->acm_lut_hue_table;
				local_acm_lut_sata_table = cur_acm_reg->acm_lut_sata_table;
				local_acm_lut_satr0_table = cur_acm_reg->acm_lut_satr0_table;
				local_acm_lut_satr1_table = cur_acm_reg->acm_lut_satr1_table;
				local_acm_lut_satr2_table = cur_acm_reg->acm_lut_satr2_table;
				local_acm_lut_satr3_table = cur_acm_reg->acm_lut_satr3_table;
				local_acm_lut_satr4_table = cur_acm_reg->acm_lut_satr4_table;
				local_acm_lut_satr5_table = cur_acm_reg->acm_lut_satr5_table;
				local_acm_lut_satr6_table = cur_acm_reg->acm_lut_satr6_table;
				local_acm_lut_satr7_table = cur_acm_reg->acm_lut_satr7_table;
...
#if defined(CONFIG_HISI_FB_3660) || defined(CONFIG_HISI_FB_970)
	if ((g_dss_version_tag == FB_ACCEL_KIRIN970_ES) || (g_dss_version_tag == FB_ACCEL_HI366x)) {
...
void hisi_dss_mctl_on(struct hisi_fb_data_type *hisifd, int mctl_idx, bool enable_cmdlist, bool fastboot_enable)
{
	...
	if (enable_cmdlist) {
		...
		if (g_dss_version_tag == FB_ACCEL_KIRIN970) {
		 ...
		 #endif
		} else if (g_dss_version_tag == FB_ACCEL_KIRIN660) {
		...
		if (g_dss_version_tag == FB_ACCEL_KIRIN660) {
	  ...
	  dss_overlay_block_t *pov_h_block, dss_rect_t *wb_ov_block_rect, int ovl_idx, int ov_h_block_idx)
{
	...
	ovl_bg_color = (g_dss_version_tag == FB_ACCEL_KIRIN970) ? 0x3FF00000 : 0xFFFF0000;
#else
	...
	char __iomem *wdma_base, dss_wdma_t *s_wdma)
{
	...
	if (g_dss_version_tag == FB_ACCEL_KIRIN970_ES) {
		hisifd->set_reg(hisifd, wdma_base + ROT_SIZE_ES, s_wdma->rot_size, 32, 0);
	} else {
