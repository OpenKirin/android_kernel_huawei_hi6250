	...
	char __iomem *scl_lut_base, int coef_lut_idx)
{
	...
	if (g_dss_version_tag == FB_ACCEL_KIRIN970_ES){
		ret = hisi_dss_arsr1p_write_lsc_gain(hisifd, enable_cmdlist, scl_lut_base + ARSR1P_LSC_GAIN_ES,
			(const uint32_t **)ARSR1P_LSC_GAIN_TABLE, LSC_ROW, LSC_COL);
		if (ret < 0) {
			HISI_FB_ERR("Error to write lsc gain.\n");
		}

		ret = hisi_dss_arsr1p_write_coefs(hisifd, enable_cmdlist, scl_lut_base + ARSR1P_COEFF_H_Y0_ES,
			(const int **)COEF_Y_SCL_UP_TAP6, ARSR2P_PHASE_NUM, ARSR2P_TAP6);
		if (ret < 0) {
			HISI_FB_ERR("Error to write H_Y0_COEF coefficients.\n");
		}

		ret = hisi_dss_arsr1p_write_coefs(hisifd, enable_cmdlist, scl_lut_base + ARSR1P_COEFF_V_Y0_ES,
			(const int **)COEF_Y_SCL_UP_TAP6, ARSR2P_PHASE_NUM, ARSR2P_TAP6);
		if (ret < 0) {
			HISI_FB_ERR("Error to write V_Y0_COEF coefficients.\n");
		}

		ret = hisi_dss_arsr1p_write_coefs(hisifd, enable_cmdlist, scl_lut_base + ARSR1P_COEFF_H_UV0_ES,
			(const int **)COEF_AUV_SCL_UP_TAP4, ARSR2P_PHASE_NUM, ARSR2P_TAP4);
		if (ret < 0) {
			HISI_FB_ERR("Error to write H_UV0_COEF coefficients.\n");
		}

		ret = hisi_dss_arsr1p_write_coefs(hisifd, enable_cmdlist, scl_lut_base + ARSR1P_COEFF_V_UV0_ES,
			(const int **)COEF_AUV_SCL_UP_TAP4, ARSR2P_PHASE_NUM, ARSR2P_TAP4);
		if (ret < 0) {
...
int hisi_dss_arsr2p_config(struct hisi_fb_data_type *hisifd, dss_layer_t *layer, dss_rect_t *aligned_rect, bool rdma_stretch_enable)
{
	...
	if ((src_width < ARSR2P_MIN_INPUT) || (dst_rect.w < ARSR2P_MIN_INPUT) ||
		...
		if ((!en_hscl) && (!en_vscl) && sharpen_en) {
			...
			HISI_FB_ERR("fb%d, src_rect.w(%d) or dst_rect.w(%d) is smaller than 16 or larger than 2560!\n",
				hisifd->index, src_width, dst_rect.w);
			return -EINVAL;
		}
	}
...
void hisifb_dss_disreset(struct hisi_fb_data_type *hisifd)
{
	...
	if (g_dss_version_tag == FB_ACCEL_KIRIN970) {
		...
		outp32(media_crg_base + MEDIA_PEREN0, 0x0009C000); //0x2009C000
		outp32(media_crg_base + MEDIA_PEREN1, 0x00660000); //0x01E66000
		outp32(media_crg_base + MEDIA_PEREN2, 0x0000003F);
		udelay(1);

		//3: module clk disable
		outp32(media_crg_base + MEDIA_PERDIS0, 0x0009C000);//0x2009C000
		outp32(media_crg_base + MEDIA_PERDIS1, 0x00600000);//0x01E06000
		outp32(media_crg_base + MEDIA_PERDIS2, 0x0000003F);
		udelay(1);

		//4: module unrst
		outp32(media_crg_base + MEDIA_PERRSTDIS0, 0x000000C0);//0x001C40C0
		outp32(media_crg_base + MEDIA_PERRSTDIS1, 0x000000F0);

		//5: module clk enable
		outp32(media_crg_base + MEDIA_PEREN0, 0x0009C000);//0x2009C000
		outp32(media_crg_base + MEDIA_PEREN1, 0x00600000);//0x01E06000
		outp32(media_crg_base + MEDIA_PEREN2, 0x0000003F);
		...
		//DSI pu
		outp32(peri_crg_base + PERRSTDIS3, 0x10000000);
		outp32(peri_crg_base + PEREN5, 0x10000000);
	} else {
		///////////////////////////////////////////////////////////////////////////
		//
		//MEDIA_SUBSYS_PU
		...
		//
		//2: module clk enable
		outp32(media_crg_base + CLK_DIV0, 0x3FF008B);
		outp32(media_crg_base + CLK_DIV2, 0x3FF008B);
	...
	dss_rect_t *out_aligned_rect, bool *rdma_stretch_enable)
{
	...
	*rdma_stretch_enable = (stretched_line_num > 0) ? true : false;
	...
	if (layer->transform & HISI_FB_TRANSFORM_ROT_90) {
		aligned_rect.left = ALIGN_DOWN(new_src_rect.x, hfbcd_block_height_align);
		aligned_rect.right = ALIGN_UP(new_src_rect.x + new_src_rect.h, hfbcd_block_height_align) - 1;
		aligned_rect.top = ALIGN_DOWN(new_src_rect.y, hfbcd_block_width_align);
		aligned_rect.bottom = ALIGN_UP(new_src_rect.y + new_src_rect.w, hfbcd_block_width_align) - 1;
	}
	...
	if (layer->transform & HISI_FB_TRANSFORM_ROT_90) {
		...
		clip_rect->right = aligned_rect.right - DSS_WIDTH(new_src_rect.x + new_src_rect.h);
		clip_rect->top = new_src_rect.y - aligned_rect.top;
		clip_rect->bottom = aligned_rect.bottom - DSS_HEIGHT(new_src_rect.y + new_src_rect.w);
	} else {
		clip_rect->left = new_src_rect.x - aligned_rect.left;
...
{
	BUG_ON(post_scf_base == NULL); //lint !e730
	BUG_ON(s_post_scf == NULL); //lint !e730
	...
	char __iomem *post_scf_base, dss_arsr1p_t *s_post_scf)
{
	...
	if (g_dss_version_tag == FB_ACCEL_KIRIN970_ES){
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_SKIN_THRES_Y_ES, s_post_scf->skin_thres_y, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_SKIN_THRES_U_ES, s_post_scf->skin_thres_u, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_SKIN_THRES_V_ES, s_post_scf->skin_thres_v, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_SKIN_EXPECTED_ES, s_post_scf->skin_expected, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_SKIN_CFG_ES, s_post_scf->skin_cfg, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_SHOOT_CFG1_ES, s_post_scf->shoot_cfg1, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_SHOOT_CFG2_ES, s_post_scf->shoot_cfg2, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_SHARP_CFG1_ES, s_post_scf->sharp_cfg1, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_SHARP_CFG2_ES, s_post_scf->sharp_cfg2, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_SHARP_CFG3_ES, s_post_scf->sharp_cfg3, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_SHARP_CFG4_ES, s_post_scf->sharp_cfg4, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_SHARP_CFG5_ES, s_post_scf->sharp_cfg5, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_SHARP_CFG6_ES, s_post_scf->sharp_cfg6, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_SHARP_CFG7_ES, s_post_scf->sharp_cfg7, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_SHARP_CFG8_ES, s_post_scf->sharp_cfg8, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_SHARP_CFG9_ES, s_post_scf->sharp_cfg9, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_SHARP_CFG10_ES, s_post_scf->sharp_cfg10, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_SHARP_CFG11_ES, s_post_scf->sharp_cfg11, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_DIFF_CTRL_ES, s_post_scf->diff_ctrl, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_LSC_CFG1_ES, s_post_scf->lsc_cfg1, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_LSC_CFG2_ES, s_post_scf->lsc_cfg2, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_LSC_CFG3_ES, s_post_scf->lsc_cfg3, 32, 0);
		hisifd->set_reg(hisifd, post_scf_base + ARSR1P_FORCE_CLK_ON_CFG_ES, s_post_scf->force_clk_on_cfg, 32, 0);
	} else {
