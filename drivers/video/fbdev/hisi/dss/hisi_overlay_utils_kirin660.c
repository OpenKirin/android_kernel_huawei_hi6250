	...
	if (g_fpga_flag == 0) {
		if ((ovl_idx == DSS_OVL2) || (ovl_idx == DSS_OVL3)) {
			if (layer && ((layer->need_cap & CAP_AFBCD) != CAP_AFBCD)) {
				...
				if (credit_step > 64) {
					aif->aif_ch_hs = set_bits32(aif->aif_ch_hs, 0x0, 1, 0);   //credit en disable
				} else {
					aif->aif_ch_hs = set_bits32(aif->aif_ch_hs, 0x1, 1, 0);   //credit en enable
					aif->aif_ch_hs = set_bits32(aif->aif_ch_hs, credit_step, 7, 4);
				}
#else
				/* credit en lower */
				aif->aif_ch_ls = set_bits32(aif->aif_ch_ls, 1, 1, 0);
				aif->aif_ch_ctl = set_bits32(aif->aif_ch_ctl, 2, 4, 12);
				...
				aif->aif_ch_ls = set_bits32(aif->aif_ch_ls, 0x0, 1, 0);   //credit en disable
#endif
			}
		...
		if ((ovl_idx == DSS_OVL2) || (ovl_idx == DSS_OVL3)) {
			...
			aif->aif_ch_hs = set_bits32(aif->aif_ch_hs, 0x20, 7, 4);
#else
			/* credit en lower */
			aif->aif_ch_ls = set_bits32(aif->aif_ch_ls, 0x1, 1, 0);//credit enable lower
			aif->aif_ch_ctl = set_bits32(aif->aif_ch_ctl, 8, 4, 12);//max_cnt
			aif->aif_ch_ctl = set_bits32(aif->aif_ch_ctl, 0x2, 2, 8);	//credit step lower mode
			aif->aif_ch_ls = set_bits32(aif->aif_ch_ls, 0x0, 1, 0);//credit enable lower
#endif
		}
	}
	...
	if (g_fpga_flag == 0) {
		if ((ovl_idx == DSS_OVL0) || (ovl_idx == DSS_OVL1)) {
			...
			if (layer && (layer->need_cap & CAP_AFBCD)) {
...
#ifndef CREDIT_STEP_LOWER_ENABLE
				aif1->aif_ch_hs = set_bits32(aif1->aif_ch_hs, 0x1, 1, 0);   //credit en enable
				aif1->aif_ch_hs = set_bits32(aif1->aif_ch_hs, 0x10, 7, 4);   //credit step hs
				...
				aif1->aif_ch_ctl = set_bits32(aif1->aif_ch_ctl, 2, 4, 12);//max_cnt
				aif1->aif_ch_ls = set_bits32(aif1->aif_ch_ls, credit_step_lower, 7, 4);//credit enable lower
				aif1->aif_ch_ctl = set_bits32(aif1->aif_ch_ctl, 0x2, 2, 8);	//credit step lower mode
				aif1->aif_ch_ls = set_bits32(aif1->aif_ch_ls, 0x0, 1, 0);//credit enable lower disable
#endif
			}
		}
	} else {
		if ((ovl_idx == DSS_OVL2) || (ovl_idx == DSS_OVL3)) {
			...
			/* credit en lower */
			aif1->aif_ch_ls = set_bits32(aif1->aif_ch_ls, 0x1, 1, 0);//credit enable lower
			aif1->aif_ch_ctl = set_bits32(aif1->aif_ch_ctl, 8, 4, 12);//max_cnt
			aif1->aif_ch_ls = set_bits32(aif1->aif_ch_ls, 0x16, 7, 4);//credit enable lower
			aif1->aif_ch_ctl = set_bits32(aif1->aif_ch_ctl, 0x2, 2, 8);	//credit step lower mode
			aif1->aif_ch_ls = set_bits32(aif1->aif_ch_ls, 0x0, 1, 0);//credit enable lower
#endif
		}
	}
