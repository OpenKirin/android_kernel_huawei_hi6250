...
static DRIVER_ATTR(state, 0644, NULL,  hisi_vibrator_status);
...
static int hisi_vibrator_probe(struct platform_device *pdev)
{
    ...
    ret = of_property_read_u32(pdev->dev.of_node, "vibrator-reg-on", (u32 *)&p_data->vibrator_reg_on);
    if (ret) {
        dev_err(&pdev->dev, "failed to get vibrator-reg-on\n");
        goto err;
    }

    ret = of_property_read_u32(pdev->dev.of_node, "vibrator-reg-off", (u32 *)&p_data->vibrator_reg_off);
    if (ret) {
        dev_err(&pdev->dev, "failed to get vibrator-reg-off\n");
        goto err;
    }

    ret = of_property_read_u32(pdev->dev.of_node, "vibrator-bit-on", (u32 *)&p_data->vibrator_bit_on);
    if (ret) {
...
static int hisi_vibrator_resume(struct platform_device *pdev)
{
	...
	mutex_unlock(&pdata->lock);/*lint  !e455*/
...
static struct platform_driver hisi_vibrator_driver = {/*lint !e31*/
