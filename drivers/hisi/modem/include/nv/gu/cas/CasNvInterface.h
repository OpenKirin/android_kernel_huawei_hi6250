...
#define NV_ITEM_CAS_NVIM_CHR_REPORT_CTR_SIZE        (4)
...
typedef struct
{
    VOS_UINT8                           ucEncryptSpt;                           /* 普通加密模式的支持         Bit7--CMEA Bit6--ECMEA            ; 0--不支持 1--支持 */
                                                                                /* ECMEA 加密模式支持的前提是需要支持 CMEA 加密 */
    VOS_UINT8                           ucCschSigEncryptSpt;                    /* 公共信道信令扩展加密的支持 Bit7--保留 Bit6--ECMEA Bit5--REA  ; 0--不支持 1--支持 */
    VOS_UINT8                           ucDschSigEncryptSpt;                    /* 业务信道信令扩展加密的支持 Bit7--保留 Bit6--ECMEA Bit5--REA  ; 0--不支持 1--支持 */
    VOS_UINT8                           ucUiEncryptSpt;                         /* 用户信息扩展加密的支持     Bit7--ORXY Bit6--REA              ; 0--不支持 1--支持 */
...
/*****************************************************************************
 ...
 结构说明  : DTS2016030805560, TCH态滤波NV, 3515;
             DTS2016101903277, 更改滤波方案,修改NV 3515
             DTS2017031303578, 滤波生效时,迟滞方案中激活集强、中、弱三档信号的门限需要抬升,
             增加NV项,控制门限抬升差值.
*****************************************************************************/
typedef struct
{
    ...
    VOS_UINT8                           ucSingleUpMfFactor;                     /* TCH测量单次信号上升滤波因子 高4位为新值比 低4位为旧值比 默认值0x31 */
    VOS_UINT8                           ucDoubleUpMfFactor;                     /* TCH测量连续信号上升滤波因子 高4位为新值比 低4位为旧值比 默认值0x31 */

    VOS_UINT8                           ucSingleDwMfFactor;                     /* TCH测量单次信号下降滤波因子 高4位为新值比 低4位为旧值比 默认值0x17 */
    VOS_UINT8                           ucDoubleDwMfFactor;                     /* TCH测量连续信号下降滤波因子 高4位为新值比 低4位为旧值比 默认值0x17 */
    VOS_INT8                            cActSetLevelDiff;                       /* 滤波方案生效时,迟滞方案中激活集强、中、弱三档信号,门限抬升的差值,单位db */
    VOS_UINT8                           aucReserve[2];
...
/*****************************************************************************
 结构名    : CAS_NVIM_CHR_REPORT_CTR_INFO_STRU
 协议表格  :
 ASN.1描述 :
...
typedef struct
{
    ...
    VOS_UINT8                           ucRfCheckDirectRptTimerLen;             /* 资源核查主动上报定时器时长, 单位 分钟 */
}CAS_NVIM_CHR_REPORT_CTR_INFO_STRU;
