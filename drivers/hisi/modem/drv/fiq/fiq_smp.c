...
static int send_fiq(fiq_num fiq_num_t)
{
	u32 regval;
	u32 index_fiq;
	for(index_fiq = (u32)0; index_fiq < (u32)CCPU_CORE_NUM; index_fiq++)
	{
		...
		{
			writel((u32)0x1 << g_fiq_ctrl.per_cpu_fiq[index_fiq].sysctrl_fiq_enable_bit, (volatile void *)g_fiq_ctrl.per_cpu_fiq[index_fiq].sysctrl_fiq_enable_reg);
		}
		else /*PHONE*/
		{
			regval = (u32)readl((volatile const void *)(g_fiq_ctrl.per_cpu_fiq[index_fiq].sysctrl_fiq_enable_reg));
			regval &= (~((u32)1 << g_fiq_ctrl.per_cpu_fiq[index_fiq].sysctrl_fiq_enable_bit));
			writel(regval,(volatile void *)g_fiq_ctrl.per_cpu_fiq[index_fiq].sysctrl_fiq_enable_reg);
		}
		/*check R8 recieve FIQ*/
		udelay(1000);/*lint !e747 !e774 !e778*/
		...
		if((readl((volatile const void *)g_fiq_ctrl.per_cpu_fiq[index_fiq].smem_send_cnt_addr)) != (readl((volatile const void *)g_fiq_ctrl.per_cpu_fiq[index_fiq].smem_recive_cnt_addr)))
		{
			pr_err("[fiq%d]fiq send cnt != fiq receive cnt, lost modem fiq respond error\n",index_fiq);
		}
		udelay(1000);/*lint !e747 !e774 !e778*/
		...
		{
			pr_err("[fiq%d]smem clear value != smem status value, modem fiq handler error\n",index_fiq);
			continue;
		}
		return BSP_OK;
	}
