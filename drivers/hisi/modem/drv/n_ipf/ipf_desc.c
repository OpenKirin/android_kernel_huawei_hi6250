...
void ipf32_get_dlrd(unsigned int* pu32Num, IPF_RD_DESC_S *pstRd)
{
    ...
    for(i = 0; i < u32Num; i++)
    {
            pBuff = (void *)dma_map_single(g_ipf_ctx.dev, (void *)((char *)g_ipf_ctx.dl_info.pstIpfRDQ + u32RdqRptr*sizeof(ipf_rd_s)), sizeof(ipf_rd_s), DMA_FROM_DEVICE);
        ...
        g_ipf_ctx.status->rd_len_update += pstRd[i].u16PktLen;
            dma_unmap_single(g_ipf_ctx.dev, (dma_addr_t)pBuff, sizeof(ipf_rd_s), DMA_FROM_DEVICE);
    }
