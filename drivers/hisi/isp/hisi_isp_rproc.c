...
static int hisp_clock_enable(struct hisi_isp_nsec *dev, int clk)
{
    ...
    if ((ret = clk_set_rate(dev->ispclk[clk], (unsigned long)dev->ispclk_value[clk])) < 0) {
...
static void hisp_clock_disable(struct hisi_isp_nsec *dev, int clk)
{
    ...
    if (dev->clk_powerby_media) {
        switch (clk) {
            case ISPCPU_CLK:
            case ISPFUNC_CLK:
                if ((ret = clk_set_rate(dev->ispclk[clk], (unsigned long)dev->clkdis_dvfs[clk])) < 0) {
                    pr_err("[%s] Failed: clk_set_rate.%d\n", __func__, ret);
                    return;
                }
                pr_err("[%s] %d.clk_set_rate.%d M\n", __func__, clk, dev->clkdis_dvfs[clk]);
                break;
            default:
                break;
        }
    ...
    switch(type) {
        ...
        case ISPFUNC_CLK:
            if((rate > dev->ispclk_value[type])
                || (rate == 0)) {
                pr_err("[%s] Failed : DVFS type.0x%x, %d.(%d.%08d M) > %d.(%d.%08d M)\n", __func__,
                    type, rate, rate/1000000, rate%1000000, dev->ispclk_value[type], dev->ispclk_value[type]/1000000, dev->ispclk_value[type]%1000000);
                return -EINVAL;
            }

            if ((ret = clk_set_rate(dev->ispclk[type], (unsigned long)rate)) < 0) {
                pr_err("[%s] Failed : DVFS Set.0x%x Rate.%d.(%d.%08d M)\n", __func__, type, rate, rate/1000000, rate%1000000);
                return ret;
            }

            pr_info("[%s] DVFS Set.0x%x Rate.%d.(%d.%08d M)\n", __func__, type, rate, rate/1000000, rate%1000000);
            break;
        default:
...
static int hisi_isp_nsec_getdts(struct platform_device *pdev, struct hisi_isp_nsec *dev)
{
    ...
    if ((ret = of_property_read_string_array(np, "clock-names", clk_name, dev->clock_num)) < 0) {
    ...
    if (dev->clk_powerby_media) {
        if ((ret = of_property_read_u32(np, "clk-dislevel", (unsigned int *)(&dev->clk_dislevel))) < 0 ) {
            pr_err("[%s] Failed: clk-dislevel of_property_read_u32.%d\n", __func__, ret);
            return -EINVAL;
        }
        pr_info("[%s] clk_dislevel.0x%x\n", __func__, dev->clk_dislevel);

        if ((ret = of_property_read_u32_array(np, "clkdis-dvfs", dev->clkdis_dvfs, dev->clk_dislevel)) < 0) {
    ...
    for (i = 0; i < (int)dev->clock_num; i++) {
        ...
        pr_info("[%s] ISP clock.%d.%s: %d M\n", __func__, i, clk_name[i], dev->ispclk_value[i]/1000000);
        if (dev->clk_powerby_media && i < (int)dev->clk_dislevel)
            pr_info("[%s] clkdis.%d.%s: %d M\n", __func__, i, clk_name[i], dev->clkdis_dvfs[i]/10000000);
    }
