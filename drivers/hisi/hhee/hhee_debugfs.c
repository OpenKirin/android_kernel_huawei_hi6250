...
int hhee_init_debugfs(void)
{
	...
	junk = debugfs_create_file(
			...
			&tzdbg_fops);
	if (!junk) {
		printk(KERN_ALERT "HHEE: failed to create /sys/kernel/debug/hhee/log\n");
		ret = -1;
		goto out;
	}
	...
	if (!junk) {
		printk(KERN_ALERT "debugfs_example2: failed to create /sys/kernel/debug/hhee/sum\n");
		ret = -1;
		goto out;
	}
