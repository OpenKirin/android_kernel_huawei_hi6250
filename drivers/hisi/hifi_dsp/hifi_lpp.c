...
{
	struct recv_request *recv = NULL;
	struct recv_request *recv_pos = NULL;
	struct misc_recmsg_param *recmsg = NULL;
	unsigned int wake_cmd = (unsigned int)arg;

	spin_lock_bh(&s_misc_data.recv_proc_lock);
	list_for_each_entry(recv_pos, &recv_proc_work_queue_head, recv_node) {/*lint !e838 !e826 !e64*/
		recmsg = (struct misc_recmsg_param *)recv_pos->rev_msg.mail_buff;
		if (recmsg) {
			if (ID_AUDIO_AP_PLAY_DONE_IND == recmsg->msgID) {
				spin_unlock_bh(&s_misc_data.recv_proc_lock);
				logw("play done message has already been in work queue\n");
				return -EIO;
			}
		}
...
static int hifi_dsp_usbaudio_cmd(unsigned long arg)
{
	...
	struct usbaudio_ioctl_output output;
	struct misc_io_sync_param para;
	...
	if ((unsigned int)USBAUDIO_SET_INTERFACE == input.msg_type) {
		/* set interface */
		switch(input.input1)
		{
		case 0:
			output.out = usbaudio_ctrl_set_pipeout_interface(input.input2);
			break;
		case 1:
			output.out = usbaudio_ctrl_set_pipein_interface(input.input2);
			break;
		default:
			output.out = ERROR;
			loge("input type error. %x \n", input.input1);
			break;
		}
	} else if ((unsigned int)USBAUDIO_QUERY_CONTROLLER == input.msg_type) {
		/* query controller state */
		output.out = usbaudio_ctrl_query_controller_location();
	} else {
		/* do nothing */
		output.out = ERROR;
	...
	if (try_copy_to_user((void __user *)para_addr_out, &output, sizeof(struct usbaudio_ioctl_output))) {
		loge("copy data to user fail\n");
		ret = ERROR;
	}
error1:
...
static int hifi_misc_probe (struct platform_device *pdev)
{
	...
	memset(s_misc_data.hifi_priv_base_virt + (HIFI_AP_NV_DATA_ADDR - HIFI_UNSEC_BASE_ADDR), 0, HIFI_AP_NV_DATA_SIZE);
