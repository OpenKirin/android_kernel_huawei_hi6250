...
static void set_hmp_thresholds(unsigned int tsh_up, unsigned int tsh_down)
{
	...
	trace_set_hmp_thresholds(current, hmp_down_threshold, hmp_up_threshold);
...
{
	int plcon = 0;
	unsigned long inputlen = strlen(pname);
		...
		unsigned int up_thresholds, unsigned int down_thresholds)
{
	...
	if (up_thresholds > MAX_THRESHOLDS) {
		pr_err("hmpth--err_input--up");
		goto err_input;
	}
	if (down_thresholds > MAX_THRESHOLDS) {
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	while ((print_cnt < MAX_MUM_POLICY)
		...
		buf += snprintf(buf, (unsigned long)49, "%16s, %3d, %4d, %4u, %4u, %6d\n",
	...
	buf += snprintf(buf, (unsigned long)41, "%33u, %4u\n",
		...
		struct kobj_attribute *attr, const char *buf, size_t n)
{
	...
	while ((len > 0) && (args_num < 5)) {
		p = memchr(tmp, ',', len);
		args_len = p ? (unsigned long)(p - tmp) : 0;
...
err_store:
	...
	pr_err("args_num: %u, args_len: %u, n: %u.\n",	\
		args_num, (unsigned int)args_len, (unsigned int)n);
	return (int)n;
}
