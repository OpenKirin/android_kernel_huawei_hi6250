...
static int hi64xx_dump_ram_by_type(char *filepath, unsigned int type)
{
	...
	switch (type) {
		case DUMP_TYPE_WHOLE_OCRAM:
			snprintf(dump_ram_path, sizeof(dump_ram_path) - 1, "%s%s%s", filepath, OM_HI64XX_DUMP_RAM_LOG_PATH, OM_HI64XX_DUMP_OCRAM_NAME);/* [false alarm]:snprintf is safe  */
			dump_addr = dsp_priv->dsp_config.ocram_start_addr;
			dump_size = dsp_priv->dsp_config.ocram_size;
			break;
		case DUMP_TYPE_WHOLE_IRAM:
			snprintf(dump_ram_path, sizeof(dump_ram_path) - 1, "%s%s%s", filepath, OM_HI64XX_DUMP_RAM_LOG_PATH, OM_HI64XX_DUMP_IRAM_NAME);/* [false alarm]:snprintf is safe  */
			dump_addr = dsp_priv->dsp_config.itcm_start_addr;
			dump_size = dsp_priv->dsp_config.itcm_size;
			break;
		case DUMP_TYPE_WHOLE_DRAM:
			snprintf(dump_ram_path, sizeof(dump_ram_path) - 1, "%s%s%s", filepath, OM_HI64XX_DUMP_RAM_LOG_PATH, OM_HI64XX_DUMP_DRAM_NAME);/* [false alarm]:snprintf is safe  */
			dump_addr = dsp_priv->dsp_config.dtcm_start_addr;
			dump_size = dsp_priv->dsp_config.dtcm_size;
			break;
		case DUMP_TYPE_PRINT_LOG:
			snprintf(dump_ram_path, sizeof(dump_ram_path) - 1, "%s%s%s", filepath, OM_HI64XX_LOG_PATH, DUMP_LOG_FILE_NAME);/* [false alarm]:snprintf is safe  */
			dump_addr = dsp_priv->dsp_config.dump_log_addr;
			dump_size = dsp_priv->dsp_config.dump_log_size;
			break;
		case DUMP_TYPE_PANIC_LOG:
			snprintf(dump_ram_path, sizeof(dump_ram_path) - 1, "%s%s%s", filepath, OM_HI64XX_LOG_PATH, DUMP_OCRAM_FILE_NAME);/* [false alarm]:snprintf is safe  */
			dump_addr = dsp_priv->dsp_config.dump_ocram_addr;
			dump_size = dsp_priv->dsp_config.dump_ocram_size;
			break;
		case DUMP_TYPE_REG:
			snprintf(dump_ram_path, sizeof(dump_ram_path) - 1, "%s%s%s", filepath, OM_HI64XX_LOG_PATH, DUMP_REG_FILE_NAME);/* [false alarm]:snprintf is safe  */
			ret = hi64xx_save_reg_file(dump_ram_path);
			if (ret != 0)
				HI64XX_DSP_ERROR("save reg dump file fail, ret:%d\n", ret);

			return ret;
		default:
			HI64XX_DSP_ERROR("dump type err\n");
			return -EINVAL;
	}
