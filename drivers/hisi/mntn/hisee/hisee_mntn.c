...
static int	 g_collect_vote_over_flag = HISEE_ERROR;/*flag that make sure we have got vote value from lpm3*/
...
int rdr_hisee_thread(void *arg)
{
	while (!kthread_should_stop()) {
		...
		case HISEE_STATE_HISEE_EXC:
			...
			pr_err("fi[0x%x] fv[0x%x] ss[0x%x] sc[0x%x] vote lpm3[0x%x] vote atf[0x%x]\n", g_msg.data[2], g_msg.data[3], g_msg.data[4], g_msg.data[5], g_msg.data[6], g_vote_val_atf);
	...
	void *msg)
{
	...
	case HISEE_VOTE_RES:
		...
		g_collect_vote_over_flag = HISEE_OK;
...
int hisee_mntn_debug_dump(void)
{
	...
	if (HISEE_STATE_INVALID == g_hisee_mntn_state) {
...
{
	g_collect_vote_over_flag = HISEE_ERROR;
...
/********************************************************************
Description:	whether we have got vote value
input:	NA
output:	NA
return:	vote value
********************************************************************/
int hisee_mntn_vote_value_ready(void)
{
	return g_collect_vote_over_flag;
}
/********************************************************************
