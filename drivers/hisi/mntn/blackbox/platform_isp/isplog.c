...
#define CORE_DUMP_EXC_CUR   (0x1)
#define CORE_DUMP_ION          (0x2)
#define  CORE_DUMP_RTOS_FINISH    (0x4)
#define CORE_DUMP_ALL_FINISH   (0x8)
...
#define LOG_COREDUMP_START       _IOWR(ISP_IOCTL_MAGIC, 0x01, struct log_user_para) //save ion address and set ion flag
#define LOG_COREDUMP_WRITE       _IOWR(ISP_IOCTL_MAGIC, 0x02, struct log_user_para)  //set all finised flag
...
void wait_firmware_coredump(void)
{
    ...
    do {
        ...
        hisp_unlock_sharedbuf();
        ret = wait_event_timeout(dev->wait_ctl,0, msecs_to_jiffies(10 * POLLING_TIMEOUT_NS));//lint !e666
        if (0 == ret)
            pr_debug("[coredump][%s] wait timeout, ret = %d\n", __func__, ret);
              ...
              unsigned long args)
{
    ...
    case LOG_WR_OFFSET:
        ...
        if(dev->share_para->exc_flag > 0)/*bit 0:exc cur ;bit 1:ion flag ; bit 2:rtos dump over  bit3:handle over*/
        {
            if((dev->share_para->exc_flag == CORE_DUMP_EXC_CUR)||
                    (dev->share_para->exc_flag == (CORE_DUMP_EXC_CUR|CORE_DUMP_ION|CORE_DUMP_RTOS_FINISH)))
            {
                ...
                pr_err("[%s][%d][coredump] wait exc signal. exc_flag=%d\n", __func__,__LINE__,dev->share_para->exc_flag);
        ...
        break;
     case LOG_COREDUMP_START:
        if ((ret = copy_from_user(&tmp, (void __user *)args, (int)sizeof(tmp))) < 0) {
             pr_err("[%s] Failed : copy_from_user.%d\n", __func__, ret);
             return -EFAULT;
        }
        pr_err("[%s][coredump]  coredump_addr[0x%x] share_para[%pK]\n",__func__,tmp.coredump_addr,dev->share_para);
        dev->share_para->coredump_addr = tmp.coredump_addr;
        dev->share_para->exc_flag |= CORE_DUMP_ION;//set ion bit
        tmp.log_write = dev->share_para->log_cache_write;
        tmp.log_head_size = dev->share_para->log_head_size;
        break;
     case LOG_COREDUMP_WRITE:
        if(dev->share_para->exc_flag & CORE_DUMP_RTOS_FINISH)
        {
            tmp.log_write = dev->share_para->log_cache_write;
            tmp.log_head_size = dev->share_para->log_head_size;
            dev->share_para->exc_flag |= CORE_DUMP_ALL_FINISH;//set all finished
            pr_err("[%s][%d][coredump]  ion set all_finished bit success\n",__func__,__LINE__);
        }
        break;
    default:
