...
static irqreturn_t hi64xx_sound_trigger_handler(int irq, void *data)
{
	...
	pr_info("%s,soundtrigger_event = %d, fast_mode_enable = %d, FINDPHONE = %d\n",
		__FUNCTION__, soundtrigger_event, vad_data->fast_mode_enable, ST_EVENT_COMMAND_FINDPHONE);

	if (soundtrigger_event == ST_EVENT_COMMAND_FINDPHONE) {
		soundtrigger_event = 6;
		hw_soundtrigger_event_uevent(soundtrigger_event);
		pr_info("soundtrigger input findphone[%d].\n",soundtrigger_event);
	} else {
		if (0 == vad_data->fast_mode_enable) {
			soundtrigger_event = 0;
