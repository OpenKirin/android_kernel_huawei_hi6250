...
extern int hisee_mntn_vote_value_ready(void);
...
#ifdef CONFIG_HISEE_CHIPTEST_SUPPORT_APPLET_OPTIMIZATION
	unsigned char cmd3[13] = {0x80, 0xe4, 0x00, 0x80,0x07, \
...
static int rpmb_ready_body(void *arg)
{
	...
	*(volatile unsigned int *)(HISEE_STATE_ADDR) = value; /*lint !e747*/
	...
	if (0 == retry || HISEE_OK != find_part_res) {
		...
		(void)hisee_cos_patch_read(MISC3_IMG_TYPE);
#endif
		access_hisee_image_partition((char *)&misc_version, MISC_VERSION_READ_TYPE);
		pr_err("%s, COS MISC version=%x \n", __func__, misc_version.img_version_num[0]);
	}
...
static ssize_t hisee_power_debug_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (HISEE_ERROR == hisee_mntn_vote_value_ready()) {
		hisee_mntn_collect_vote_value_cmd();
		snprintf(buff, HISEE_ERROR_DESCRIPTION_MAX, "Please cat it again\n");
	} else {
