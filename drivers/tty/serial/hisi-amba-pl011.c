...
void uart_clk_resetreg_print(struct uart_amba_port *uap){
	u32 reg;

	if(uap->reset_clk_reg_base){
		reg = readl(uap->reset_clk_reg_base + uap->reset_clk_reg_offset);
		dev_info(uap->port.dev, "enable reset_reg print,the reset_register value is 0x%x \n", reg);
	}
}
