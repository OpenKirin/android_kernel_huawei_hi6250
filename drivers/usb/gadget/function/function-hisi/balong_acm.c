				...
				size_t count, loff_t *ppos)
{
	...
	port->stat_read_call++;
	if (unlikely(NULL == buf || 0 == count)) {
		pr_err("%s: param err, buf %pK, count %ld\n",
					__func__, buf, count);
		port->stat_read_param_err++;
		return -EFAULT;
	}
...
int gs_acm_ioctl(unsigned int port_num, unsigned int cmd, unsigned long arg)
{
	...
	case ACM_IOCTL_WRITE_ASYNC:
		...
		status = gs_acm_cdev_write_base(port, rw_info->pVirAddr, rw_info->pPhyAddr, rw_info->u32Size, false);
		break;
...
int gs_acm_cdev_alloc_line(unsigned char *line_num)
{
	...
	gs_acm_cdev_ports[port_num].cdev = cdev;/*  [false alarm]:not a bug */
...
int gacm_cdev_line_state(struct gserial *gser, u8 port_num, u32 state)
{
	...
	if (!gs_cdev_driver || port_num >= gs_acm_cdev_n_ports) {
		pr_emerg("gacm_cdev_connect fail drv:%pK, port_num:%d, n_ports:%d\n",
...
#define ACM_PRINT_IOCTL(cmd, seq_file) seq_printf(seq_file, "comand:%s\t\t\t\tcode:0x%x\n", #cmd, cmd);
...
#define ACM_PRINT_ERRNO(err, seq_file) seq_printf(seq_file, "%s\t\t\t\t%d\t0x%08x\n", #err, -err, -err);
...
void hiusb_do_acm_cdev_dump(struct seq_file *s, unsigned int port_num)
{
	...
	if (!gs_cdev_driver || port_num >= gs_acm_cdev_n_ports) {
		seq_printf(s, "gacm_dump fail drv:%pK, port_num:%d, n_ports:%d\n",
				 gs_cdev_driver, port_num, gs_acm_cdev_n_ports);
		return ;
	}
